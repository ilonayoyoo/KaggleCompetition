---
title: "Kaggle Car Sale Prediction"
author: "You Wu"
date: "2023-12-07"
output:
...
```{r}
setwd("/Users/youwu/Desktop/23Fall Term1/5200/Kaggle Dataset")

#Read data and pre-processing data for both training and testing dataset
train = read.csv("analysisData.csv")
train2 = read.csv("scoringData.csv")
train <- bind_rows(train, train2)

```

-   Check missing value, including NULL, "", empty value
...
-   To facilitate filling missing value, convert all "" to NA
...
Fill all missing values with NA and count NA within each column
...
classification of missing data into three categories:
...
```         
MCAR -remvove NAs or impute, mean, mode, median
MAR - use advanced imputation methods like MICE
```

Fill NA values for new cars: T/F in accidents, owners, etc - Numeric Var: Fill NA with 0 because new car has no mileage or previous owner - T/F Var; Fill all new cars with whether have accidents, a cab, damaged with False

Also convert T/F value to numeric
...
-   Extract torque and power from its units
    ...
    |                     |                        |
    |---------------------|------------------------|
    | 145 hp \@ 4,600 RPM | 180 lb-ft \@ 3,600 RPM |
...
-   Fill in MCAR using 1. Median (for numerical value) 2.Mode (for categorical value) grouping by their trim_name
...
-   price distribution group by different brands
...
-   From the plot, we can see even for the same brand(Car Make) , there's significant difference among diferrent type of car models. So for the imputing, I will fill missing value group by the model.

-   And also split the model column into a 1,0 dummy variable matrix using one-hot encoding and append them to the dataframe as features.
...
-   True or False value missing:
    -   It turns out only the new cars have missing T/F value:

    -   isCab; has_accidents; is_cpo; salvage; frame_damaged; fleet; franchise_dealer; is_new)

    -   so it's MAR type of missing value (dependent on other variables, such as mileage, under 10 is usually a new car)
...
convert model year to number of years from today
...
-   select features and split data set to train model, since we have converted useful categorical var into numerical, we only select the numerical features into the model training and predicting
...
-   gbm model (due to time constraint, the parameter tuning part is omitted)
...
-   After tuning, the best param meters are shown below, since we will use XGBoost as our final model which has the best performance, I will set n.trees to a smaller number to accelerate the compiling speed.
...
pred = predict(boost,testData,n.trees=1000)
...
-   Split 40,000 data into 35,000 and 5,000 as train and test data and use loss function, L1 regularization hyperparameters to avoid overfitting

-   After running multiple models, select the best tuned hyperparameters and train it on the entire train data
...
-   Following hyperparameters are the result of the above training process using minimum RSME
...
pred<- predict(final_model, dpredict)
scoringData = read.csv("scoringData.csv")
submissionFile = data.frame(id = scoringData$id, price = round(pred))
